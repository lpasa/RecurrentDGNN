...
from model.graphConvNet import GraphConv_GNN
...
if __name__ == '__main__':
    ...
    test_epoch = 1
    aggregator = 'concat'
    test_name = "DeepGraphConv_Baseline_Test"

    dataset_path = '~/Dataset/MUTAG'
    dataset_name = 'MUTAG'

    test_name = test_name + "_data-" + dataset_name + "_aggregator-" + aggregator + "_nFold-" + str(
        n_folds) + "_lr-" + str(lr) +"_drop_prob-"+str(drop_prob)+"_weight-decay-"+ str(weight_decay)+ "_batchSize-" + str(
        batch_size) + "_nHidden-" + str(n_units) + "_maxK-" + str(max_k)
    ...
    for split_id, split in enumerate(dataset_cv_splits):
        ...
        model = GraphConv_GNN(loader_train.dataset.num_node_labels, n_units, n_classes, drop_prob , max_k=max_k).to(device)
